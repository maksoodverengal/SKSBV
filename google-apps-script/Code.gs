...
function doPost(e) {
  try {
    ...
    if (!sheet) throw new Error('Sheet "Responses" was not found.');
    ...
    if (allowed.indexOf(mimeType) === -1) throw new Error("Invalid image type.");

    // Apps Script has execution/request limits; keeping the website limit at
    // 2 MB makes this suitable for small/normal registration forms.
    ...
    const fileName = new Date().getTime() + "_" + safeName + extension;
